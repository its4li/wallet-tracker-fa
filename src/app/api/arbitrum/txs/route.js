...
import { fetchArbitrumTransactions } from "../../../../lib/arbiscan";
  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { message: "کلید API Arbiscan تنظیم نشده است." },
        { status: 500 }
      );
    }

    const { ok, result, error } = await fetchArbitrumTransactions({
      address, page, pageSize, apiKey
    });

    if (!ok) {
      return NextResponse.json({ message: error || "خطا در دریافت اطلاعات از شبکه Arbitrum" }, { status: 502 });
    }
    ...
    console.error("Arbitrum API Error:", e);
