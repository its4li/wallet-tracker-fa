...
import { fetchOptimismTransactions } from "../../../../lib/optimistic-etherscan";
  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { message: "کلید API Optimistic Etherscan تنظیم نشده است." },
        { status: 500 }
      );
    }

    const { ok, result, error } = await fetchOptimismTransactions({
      address, page, pageSize, apiKey
    });

    if (!ok) {
      return NextResponse.json({ message: error || "خطا در دریافت اطلاعات از شبکه Optimism" }, { status: 502 });
    }
    ...
    console.error("Optimism API Error:", e);
